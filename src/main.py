...
def main() -> int:
    ...
    try:
        ...
        github_client = GitHubClient(token=settings.github_token, repository=settings.repository)
        ...
        comment_body = formatter.format_comment(result=result, total_files=len(result.key_files))

        # Post or update comment
        logger.info("Posting analysis comment to PR")
        github_client.post_or_update_comment(pr_number=settings.pr_number, body=comment_body)
        ...
        try:
            ...
            github_client.post_or_update_comment(pr_number=settings.pr_number, body=error_comment)
        except Exception:
